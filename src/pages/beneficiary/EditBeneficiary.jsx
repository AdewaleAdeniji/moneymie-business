...
const EditBeneficiary = (props) => {
    ...
    const { data: beneficiary } = useQuery('beneficiary', () => request({ url: `/beneficiary/company/${id}` }), {
    ...
    const editBeneficiary = async (values, onSubmitProps) => {
        ...
        try {
            await request({ url: `/beneficiary/company/${id}`, method: 'POST', data: body })
                    ...
                    <Form>
                        ...
                        <div className="form-fields add-ben">
                            ...
                            <FormField title="Bank Country">
                                <SelectCountry value={beneficiary?.bank_country} onChange={(e) => { setBankCountry(e.target.value) }} />
                            </FormField>

                        </div>
                        <div className="form-fields add-ben">
                            <LargeButton type="submit" disabled={formik.isSubmitting}>
                                {formik.isSubmitting ? <CircularProgress size={6} isIndeterminate color='green.300' /> : 'Submit'}
                            </LargeButton>
                        </div>
                    </Form>
                )}
